...
@room_router.get_all("/", status_code=status.HTTP_200_OK)
async def get_all(db: Session, skip, limit):
    return RoomServices.get_all_rooms(db, skip, limit)

@room_router.get_room("/", status_code=status.HTTP_200_OK)
...
@room_router.put("/", status_code=status.HTTP_201_CREATED)
